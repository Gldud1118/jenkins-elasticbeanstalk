pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                echo 'Deploying'
                sh 'aws elasticbeanstalk create-application-version --region ap-northeast-2 --application-name jenkins-test --version-label ${PROJECT_NAME}-${BUILD_NUMBER} --description ${BUILD_TAG} --source-bundle S3Bucket="aws-beanstalk-deploy",S3Key="${JOB_NAME}-${GIT_BRANCH}-${BUILD_NUMBER}.war"' 
